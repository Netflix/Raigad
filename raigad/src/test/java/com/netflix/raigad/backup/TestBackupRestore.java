...
public class TestBackupRestore extends ElasticsearchIntegrationTest {
    ...
    public void testSimpleWorkflow() throws Exception {
        ...
        Assert.assertEquals(client0.prepareCount("test-idx-1").get().getCount(), 100L);
        Assert.assertEquals(client0.prepareCount("test-idx-3").get().getCount(), 100L);

        //Run backup
        snapshotBackupManager.runSnapshotBackup();

        Assert.assertEquals(client0.admin().cluster().prepareGetSnapshots(repositoryName)
                .setSnapshots(snapshotBackupManager.getSnapshotName("_all", false))
        ...
        Assert.assertEquals(client0.prepareCount("test-idx-1").get().getCount(), 50L);
        Assert.assertEquals(client0.prepareCount("test-idx-3").get().getCount(), 50L);
